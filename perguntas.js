criaCartao(
    'matemática',
    'Qual é a fórmula de Bhaskara?',
    '-b±√Δ/2'
)

criaCartao(
    'matemática',
    'Qual a fórmula base de uma Função Afim?',
    'f(x)=ax+b',
)

criaCartao(
    'matemática',
    'Qual é o cosseno de 60°?',
    '1/2'
)

criaCartao(
    'matemática',
    'Quais são os 3 primeiros números de π?',
    '3,14'
)

criaCartao(
    'português',
    'Qual a forma de linguagem em que só são usadas imagens?',
    'Não verbal'
)

criaCartao(
    'português',
    'Qual o tipo de texto usado na redação do ENEM?',
    'Texto dissertativo-argumentativo'
)

criaCartao(
    'português',
    'O que é uma charge?',
    'É um gênero textual jornalístico marcado pelo humor e pela ironia. Utiliza caricaturas para fazer crítica social, política, esportiva etc.'
)

criaCartao(
    'português',
    'Qual a diferença entre charge e tirinha?',
    'É a quantidade de quadros. A charge apresenta apenas um, bastante impactante, já a tirinha traz dois ou mais quadros e uma história a ser acompanhada.'
)

criaCartao(
    'geografia',
    'O que é capitalismo?',
    'É um sistema econômico que e baseia na propriedade privada, no lucro e no trabalho assalariado.'
)

criaCartao(
    'geografia',
    'Em que período aconteceu a Primeira revolução Industrial?',
    'De 1760 até 1850'
)

criaCartao(
    'geografia',
    'O que é socialismo?',
    'O socialismo é uma ideologia política e econômica que busca a propriedade coletiva dos meios de produção e a distribuição equitativa de recursos.'
)

criaCartao(
    'geografia',
    'O que o comunismo defende?',
    'O comunismo defende a abolicação da propriedade privada, a criação de uma sociedade sem classes e a distribuição igualitária de bens e recursos.'
)

criaCartao(
    'história',
    'Em que período aconteceu a Ditadura Militar no Brasil?',
    'De 1964 até 1985'
)

criaCartao(
    'história',
    'Qual foi o primeiro preidente do Brasil?',
    'Deodoro da Fonseca'
)

criaCartao(
    'história',
    'Qual foi outro país envolvido na guerra do Brasil e do Paraguai?',
    'Uruguai'
)

criaCartao(
    'história',
    'Quem foi Getúlio Vargas?',
    'Foi um presidente do Brasil que governou de 1930 a 1945 e de 1951 a 1954, conhecido por reformas trabalhistas e políticas importantes.'
)

criaCartao(
    'biologia',
    'O que é uma célula?',
    'É a menor unidade funcional dos seres vivos, responsável por realizar as atividades vitais. Pode ser procarionte ou eucarionte.'
)

criaCartao(
    'biologia',
    'Qual é a função dos cloroplastos?',
    'São organelas responsáveis pela fotossíntese, processo em que a luz solar é convertida em energia química.'
)

criaCartao(
    'biologia',
    'Qual é o material genético presente nas células?',
    'É o DNA, que contém as informações necessárias para o desenvolvimento e funcionamento dos organismos.'
)

criaCartao(
    'biologia',
    'O que é uma cadeia alimentar?',
    'É uma sequência de organismos onde um serve de alimento para o outro, mostrando o fluxo de energia entre os seres vivos.'
)

//ao todo sao 20 perguntas de memorização do ensino médio
//matematica, port, geo, hist, bio 4 de cada